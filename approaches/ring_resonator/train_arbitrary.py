"""
Ring Resonator Training: Arbitrary Strong Pulses (Batched)
==========================================================
Batch-aware inverse design of per-loop strong pulses in a ring resonator via
XPM. Each resonator step has its own independent strong pulse parameterized
in the HG basis (N_resonator x N_modes complex parameters).  Optimizes all
pulses simultaneously to preserve multiple HG modes.
...
Usage:
    python -m approaches.ring_resonator.train_arbitrary --config approaches/ring_resonator/configs/config_ring_arbitrary_fidelity.yaml
    python -m approaches.ring_resonator.train_arbitrary --num_modes 4 --N_train 50  # CLI only
    python -m approaches.ring_resonator.train_arbitrary --config approaches/ring_resonator/configs/config_ring_arbitrary_trace.yaml --lr 1e-4  # config + CLI override
"""
...
from nlse.losses import fid_phase_loss, trace_loss
from approaches.ring_resonator.optics import beam_splitter
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        parents=[pre],
        description="Ring Resonator Arbitrary Strong-Pulse Training (Batched)",
...
def main():
    ...
    hg_loss = {"fidelity": fid_phase_loss, "trace": trace_loss}[args.loss_fn]

    def loss_function(final_j):
        return hg_loss(_final_hg(final_j), y_hg)
    ...
    def forward(A_k_hg_stack_param):
        ...
        Each resonator loop uses its own independent strong pulse converted
        from A_k_hg_stack_param[i].
        ...
        for i in range(args.N_resonator):
            A_k_time = hg_to_time(A_k_hg_stack_param[i], hg_basis)  # (Nt,)

            # Batched propagation: A_j (num_modes, Nt), A_k_time (Nt,)
            A_j_ev, _ = split_step_fourier_xpm_batch(
                A_j, A_k_time, dz, args.Nz,
    ...
    print(f"Trainable parameters: A_k_hg_stack of shape {tuple(A_k_hg_stack.shape)} "
          f"({args.N_resonator} loops x {args.N_modes} HG coefficients)")
    ...
    for i in tqdm(range(args.N_train), desc="Training"):
        optimizer.zero_grad()
        A_j_out = forward(A_k_hg_stack)          # (num_modes, Nt)
    ...
    # -------------------------------------------------------------------
    with torch.no_grad():
        A_j_final = forward(A_k_hg_stack)  # (num_modes, Nt)
    ...
    torch.save({
        "scenario": "arbitrary",
        "A_k_hg_stack": A_k_hg_stack.detach().cpu(),
    ...
    for loop_idx in range(args.N_resonator):
        ...
        color = cmap_loops(loop_idx / args.N_resonator)
    ...
    # 3. Strong pulse evolution across resonator loops
    fig = plt.figure(figsize=(12, 7), dpi=args.dpi)
    cmap_pulses = plt.cm.plasma
    n_pulses = len(A_k_stack)
    colors = [cmap_pulses(i / n_pulses) for i in range(n_pulses)]
    t_np = t.cpu().numpy()
    ...
    # 4. HG coefficient comparison (magnitude + phase) via plot_mode_hg_coeffs
    ...
    plot_mode_hg_coeffs(
        ...
        num_modes=num_modes,
        transformation_name="Ring Resonator Arbitrary (identity)",
    )
