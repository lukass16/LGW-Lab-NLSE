...
This directory contains a Python training script for the XPM Identity Operator that can be run as a batch job.

## Files

- `train_xpm_identity.py`: Main training script
...
```bash
python train_xpm_identity.py --help
```
...
```bash
python train_xpm_identity.py --config my_config.yaml
```

### Example: Custom Run Directory

```bash
python train_xpm_identity.py --run-dir runs/my_experiment
```

### Example: Without Wandb

```bash
python train_xpm_identity.py --no-wandb
```
...
python train_xpm_identity.py --config config.yaml
```
